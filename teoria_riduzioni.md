...
Le **riduzioni** sono lo strumento con cui si confronta la difficoltà di due problemi diversi, e sono il meccanismo alla base delle dimostrazioni di **NP-completezza** introdotte in [teoria_complessita](teoria_complessita.md). Intuitivamente: *ridurre $A$ a $B$* significa mostrare che, avendo a disposizione un modo per risolvere $B$, si può risolvere anche $A$ senza troppo sforzo aggiuntivo.
...
1. **Appartenenza a NP**: si esibisce un certificato $y$ e un verificatore polinomiale $V(x, y)$ (vedi [teoria_complessita §4](teoria_complessita.md#4-classe-np)).
...
Anche qui la costruzione è polinomiale ($O(m^2)$ archi al più), quindi **Clique è NP-completo** (appartenenza a NP già mostrata in [teoria_complessita §4](teoria_complessita.md#4-classe-np)).
...
> Per le definizioni di P, NP, NP-completezza e le classi correlate, vedi [teoria_complessita](teoria_complessita.md).
