...
La dimostrazione ricicla la diagonale $\varphi_x(x)$ che nel [§2.1](#21-il-problema-della-fermata) demolisce la decidibilità: qui la stessa costruzione, invece di produrre una contraddizione, produce un punto fisso — la stessa mossa usata due volte per scopi opposti, un parallelo diretto con il trucco $\varepsilon/2^n$ di [teoria_misura.md §4](teoria_misura.md#4-misura-esterna-di-lebesgue), riusato lì per la non numerabilità di $\mathbb{R}$ e per la subadditività della misura esterna.
...
È lo stesso schema diagonale usato in [teoria_misura.md §1](teoria_misura.md#1-insiemi-numerabili-e-più-che-numerabili) per dimostrare che $\mathbb{R}$ non è numerabile: si costruisce un oggetto ($c$ qui, $y$ lì) tarato apposta per differire, in almeno un punto, da ogni elemento di un'ipotetica enumerazione o di un'ipotetica decisione.
...
- B. Zoph, V. Vasudevan, J. Shlens, Q. V. Le, «Learning Transferable Architectures for Scalable Image Recognition», *CVPR*, 2018 — NASNet-A, il dato a ~2000 GPU-giorni del grafico in [§4.3](#43-il-livello-architettura-il-crollo-dei-costi-in-nas).
- E. Real, A. Aggarwal, Y. Huang, Q. V. Le, «Regularized Evolution for Image Classifier Architecture Search», *AAAI*, 2019 — AmoebaNet-A e la ricerca evolutiva.
- H. Pham, M. Guan, B. Zoph, Q. V. Le, J. Dean, «Efficient Neural Architecture Search via Parameter Sharing», *ICML*, 2018 — ENAS e il weight-sharing.
- H. Liu, K. Simonyan, Y. Yang, «DARTS: Differentiable Architecture Search», *ICLR*, 2019 — la rilassazione differenziabile e la tabella comparativa di costi del grafico in [§4.3](#43-il-livello-architettura-il-crollo-dei-costi-in-nas).
