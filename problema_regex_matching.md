...
Il ramo `a*`/`.*` è l'unico punto in cui la funzione genera più di una chiamata: da qui nasce la stessa esplosione combinatoria vista in [problema_fibonacci §1](problema_fibonacci.md#1-soluzione-ricorsione-diretta--o2n). Per vederlo, ecco il trace di `regexMatch("aab", "aa*b")` (tokens `["a", "a*", "b"]`), che restituisce `true`:
...
> (match (cons 0 0))
> ```
> È lo stesso vantaggio della tipizzazione dinamica già notato in [problema_fibonacci §2](problema_fibonacci.md#un-memoize-generico-e-riutilizzabile): Go richiede di dichiarare in anticipo la forma della chiave, Guile no.
