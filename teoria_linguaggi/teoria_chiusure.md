...
Le chiusure lessicali nascono qui: sono la primitiva su cui Scheme è costruito, non un'aggiunta successiva (vedi [fondamenti_guile §4](fondamenti_guile.md#4-definizioni-e-funzioni) per `define` e `lambda`). Il binding è mutabile direttamente con `set!`, senza bisogno di alcuna indirezione:
