# Analisi 1 $\to$ Analisi 2: ripasso essenziale
...
> **Serve per:** l'ottimizzazione libera in più variabili è la trasposizione esatta di questa sezione — $f'(x_0)=0$ diventa $\nabla f = 0$, e il segno di $f''$ diventa il segno della matrice hessiana (definita positiva $\to$ minimo, definita negativa $\to$ massimo, indefinita $\to$ sella).
