...
Questo stesso repository (`geonote`) pubblica le proprie pagine su GitHub Pages **senza** alcun workflow: come descritto in [tema_geoteo §2.4](tema_geoteo.md#24-abilitare-github-pages), basta *Settings $\to$ Pages $\to$ Deploy from a branch*, perché *Jekyll* è generato **nativamente** dalla pipeline di build di GitHub Pages. `nn-option-pricing`, che usa *Sphinx* invece di Jekyll, non ha questa scorciatoia: Sphinx non è tra i generatori supportati nativamente da GitHub Pages, quindi la build va eseguita a mano (o, meglio, automatizzata) e solo l'HTML risultante va caricato come artifact di Pages.
...
| File coinvolti       | `_layouts/`, `_config.yml` (vedi [tema_geoteo §2](tema_geoteo.md#2-passi-per-replicarlo-in-un-altro-repository)) | `.github/workflows/docs.yml`, `docs/source/` |
...
- Vedi anche [tema_geoteo](tema_geoteo.md) per la configurazione di GitHub Pages usata da questo stesso repository.
